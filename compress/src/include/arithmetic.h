...
private:
    ...
    uint64_t represent_arithmetic{};
    uint64_t scale{};
    uint64_t data_size{};
    float_t represent_float_arithmetic_data{};
...
public:
    ...
    void form_probability_distribution_from_existing_probability_table();
    ...
    std::vector< uint64_t> simple_get_result() { return {data_size, represent_arithmetic, scale}; }
    void simple_import_result(const std::vector< uint64_t > & result) {
        data_size = result[0]; represent_arithmetic = result[1]; scale = result[2];
    }
};
